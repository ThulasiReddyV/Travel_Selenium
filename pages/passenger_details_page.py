...
class Passenger_Details(Bus_and_Seat_Selection):
    ...
    def gender(self):
        ...
        gender_dp = self.wait.until(EC.visibility_of_element_located(self.GENDER_ID))
        print("Gender Selection")
        gender_dp.click()
        gender_sel = self.wait.until(EC.visibility_of_element_located(self.PASS_GENDER_SEL_XPATH))
        gender_sel.click()
    ...
    def payment_option_select(self):
        payment_option = self.wait.until(EC.visibility_of_element_located(self.PAYMENT_OPTION_XPATH))
